...
IMAGE_GEN_URL = "https://api.perplexity.ai/v1/images/generate"
MAX_LENGTH = 1000
...
def fetch_crypto_news():
    ...
    payload = {"model": "llama-3-sonar-large-32k-online", "messages": [{"role": "user", "content": prompt}]}
...
def generate_image(news_text):
    prompt = f"Create an eye-catching, non-branded image representing: {news_text[:100]} (crypto market news headline)"
    headers = {"Authorization": f"Bearer {PERPLEXITY_API_KEY}", "Content-Type": "application/json"}
...
def send_to_telegram(text, image_url):
    ...
    payload = {"chat_id": TELEGRAM_CHAT_ID, "caption": text, "photo": image_url, "parse_mode": "HTML"}
